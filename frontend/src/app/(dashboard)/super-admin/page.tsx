...
import { MOCK_ORGANIZATIONS, MOCK_PLATFORM_STATS } from '@/lib/mock-data';
...
export default function SuperAdminDashboard() {
  ...
  const statCards = [
    { label: 'Active Orgs', value: stats.active_organizations, color: 'var(--success)', icon: Building2 },
    { label: 'Riders Today', value: stats.total_riders_today, color: 'var(--brand)', icon: Users },
    { label: 'Tasks Today', value: stats.tasks_processed_today, color: 'var(--warning)', icon: ClipboardCheck },
    { label: 'Uptime', value: stats.platform_uptime, color: 'var(--success)', icon: Activity },
  ];
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-[13px] mt-0.5" style={{ color: 'var(--text-tertiary)' }}>
            {stats.active_organizations} active · {stats.suspended_organizations} suspended
          </p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                {['Organization', 'Country', 'Riders', 'Tasks / mo', 'Plan', 'Status', ''].map(h => (
            ...
            <tbody>
              {orgs.map((org, i) => (
                <tr key={org.id}
                  ...
                  style={{ borderBottom: i < orgs.length - 1 ? '1px solid var(--border-subtle)' : 'none' }}
